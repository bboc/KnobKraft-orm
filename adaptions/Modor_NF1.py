...
PATCH_MEMORY_DUMP = 0x10 # save patch in memory dump
...
Storage format is the program dump (which doesn't contain a memory location)
"""
...
def test_isMessageType():
    _failUnless(_isMessageType(TEST_MESSAGE, PATCH_DUMP_NEW_STYLE), 'message not recognized')
...
def _extract_patch_data(message):
    ...
    for idx in range(3):
        ...
        patch_data.extend(packet[7:-2]) # remove header, checksum+end
    return patch_data

def test_extract_patch_data():
    _failUnless(TEST_PATCH_DATA == _extract_patch_data(TEST_MESSAGE))
...
def test_calculate_checksum():
    _failUnless(_calculate_checksum(TEST_PATCH_DATA[0:55]) == TEST_MESSAGE[62])
    _failUnless(_calculate_checksum(TEST_PATCH_DATA[55:110]) == TEST_MESSAGE[126])
    _failUnless(_calculate_checksum(TEST_PATCH_DATA[110:]) == TEST_MESSAGE[-2])
...
def test_pack_payload():
    ...
    _failUnless(len(m) == len(TEST_MESSAGE), len(m))
    _failUnless(m == TEST_MESSAGE, _pack_payload(TEST_PATCH_DATA, PATCH_DUMP_NEW_STYLE))
    # unpack and repack needs to be the identical
    p = _extract_patch_data(TEST_MESSAGE)
    m2 = _pack_payload(p, PATCH_DUMP_NEW_STYLE)
    _failUnless(m2 == TEST_MESSAGE)
...
def test_isSingleProgramDump():
    _failUnless(isSingleProgramDump(TEST_MESSAGE))
...
F0 00 21 1C 01 0A 00 40 5F 30 47 40 40 40 6D 6D 40 40 40 40 00 01 01 00 00 00 00 04 05 7E 38 18 35 55 63 4F 31 5C 6B 62 55 4A 28 20 64 76 7F 1D 7F 40 41 33 64 3C 39 58 58 28 40 40 40 00 7C F7
"""
# def createBankDumpRequest(channel, bank):
...
# single program dump
TEST_MESSAGE = [0xf0, 0x00, 0x21, 0x1c, 0x01, 0x09, 0x00, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x34, 0x7d, 0x20, 0x31, 0x3a, 0x5c, 0x6b, 0x20, 0x4d, 0x5a, 0x6b, 0x23, 0x31, 0x5a, 0x78, 0x20, 0x64, 0x76, 0x20, 0x20, 0x7f, 0x40, 0x00, 0x30, 0x2b, 0x08, 0x00, 0x10, 0x40, 0x40, 0x40, 0x40, 0x40, 0x00, 0x3e, 0xf7,
...
TEST_PATCH_DATA = [0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x34, 0x7d, 0x20, 0x31, 0x3a, 0x5c, 0x6b, 0x20, 0x4d, 0x5a, 0x6b, 0x23, 0x31, 0x5a, 0x78, 0x20, 0x64, 0x76, 0x20, 0x20, 0x7f, 0x40, 0x00, 0x30, 0x2b, 0x08, 0x00, 0x10, 0x40, 0x40, 0x40, 0x40, 0x40, 0x00,
